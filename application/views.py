...
@api_view(['GET'])
def totalApplicant(request, format=None):

    start_date = datetime(2023, 7, 1)
    end_date = datetime(2023, 7, 31)

    application_count= Application.objects.filter(created_at__range=(start_date, end_date)).values('application_date').annotate(count=
    Count('application_id'))

    print(application_count)
    
    serializers = ApplicationCountSerializer(application_count, many=True).data
    
    return Response(serializers)


class ApplicationStatus1(generics.ListAPIView):
