...
def jobstatus(request, pk=None):
    if request.method == 'GET':
        if pk:
            ...
            job_obj=paginator.paginate_queryset(job_statuses,request)
            serializer = JobStatusSerializer(job_obj, many=True)
...
def jobposting(request, pk=None):
    if request.method == 'GET':
        ...
        if pk:
            ...
            paginator=CustomePagination()
            job_obj=paginator.paginate_queryset(jobpostings,request)
            serializer = JobPostingSerializer(job_obj, many=True)
            return paginator.get_paginated_response(serializer.data)
...
def jobstatuslog(request, pk=None):
    if request.method == 'GET':
        if pk:
            ...
            paginator=CustomePagination()
            job_obj=paginator.paginate_queryset(jobstatuslogs,request)
            serializer = JobStatusLogSerializer(job_obj, many=True)
            return paginator.get_paginated_response(serializer.data)
...
def industry(request, pk=None):
    if request.method == 'GET':
        if pk is not None:
            ...
            paginator=CustomePagination()
            industry_obj=paginator.paginate_queryset(industries,request)
            serializer = IndustrySerializer(industry_obj, many=True)
            return paginator.get_paginated_response(serializer.data)
...
def department(request, pk=None):
    if request.method == 'GET':
        if pk is not None:
            ...
            paginator=CustomePagination()
            department_obj=paginator.paginate_queryset(departments,request)
...
def skills(request, pk=None):
    if request.method == 'GET':
        if pk is not None:
            ...
            paginator=CustomePagination()
            skill_obj=paginator.paginate_queryset(skills,request)
...
def company(request, pk=None):
    if request.method == 'GET':
        if pk:
            ...
            paginator=CustomePagination()
            company_obj=paginator.paginate_queryset(companies,request)
            serializer = CompanySerializer3(company_obj, many=True)
            return paginator.get_paginated_response(serializer.data)
...
def location(request, company_id=None, location_id=None):
    if request.method == 'GET':
        if company_id:
            ...
            paginator=CustomePagination()
            location_obj=paginator.paginate_queryset(locations,request)
