...
from app.permissions import IsAdmin,IsRecruiter,IsApplicant
...
class JobPostingCountView(APIView):
    def get(self, request):
        ...
        if start_date:
            ...
            job_postings = job_postings.filter(created_at__gte=start_date)
        
        if end_date:
            end_date = datetime.strptime(end_date, '%Y-%m-%d')
            job_postings = job_postings.filter(created_at__lte=end_date)

        job_postings = job_postings.annotate(
        ...
        for job_posting in job_postings:
            month = job_posting['month'].strftime('%Y-%m')
...
class JobPostingCountByIndustryView(APIView):
    def get(self, request):
        job_postings_by_industry = JobPosting.objects.values('industry_id__industry_name').annotate(
            count=Count('job_id')
        ).order_by('-count')

        data = []
        for job_posting in job_postings_by_industry:
            industry_name = job_posting['industry_id__industry_name']
            count = job_posting['count']
            data.append({
